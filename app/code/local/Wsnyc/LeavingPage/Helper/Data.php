...
class Wsnyc_LeavingPage_Helper_Data extends Mage_Core_Helper_Abstract {
    ...
    protected function _getNewCoupon() {
        ...
        $code = null;
        if ($coupon = $this->_createCoupon($rule)) {
    ...
    protected function _createCoupon($rule)
    {
        $coupon = false;
        for($i=0; $i < 20; $i++) {
            try {
                //prevent exception when generated coupon already exists
                $coupon = $rule->acquireCoupon();
                $i = 20;
            }
            catch (Exception $e) {
                Mage::getModel('leavingpage/observer')->clearCouponCodes();
            }
        }
