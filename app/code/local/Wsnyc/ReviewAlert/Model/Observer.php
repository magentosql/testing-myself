...
class Wsnyc_ReviewAlert_Model_Observer extends Varien_Event_Observer
{
  ...
  public function sendEmail(Varien_Event_Observer $observer)
  {
    ...
    $collectionItem = $emailTemplate->getCollection()
                        ->addFieldToFilter('orig_template_code',$templatePath)
                        ->getFirstItem();

    if($collectionItem->hasData()){
      $emailTemplate->loadByCode($collectionItem->getTemplateCode());
    } else {
      $emailTemplate->loadDefault($templatePath);
    }
