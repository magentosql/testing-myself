...
class Extendware_EWMinify_Block_Override_Mage_Page_Html_Head extends Extendware_EWMinify_Block_Override_Mage_Page_Html_Head_Bridge
{
	...
	public function getCssJsHtml()
	{
		...
		if (Mage::app()->useCache('block_html')) {
			...
			$cacheKey = md5(Mage::app()->getStore()->getId() . '-' . Mage::helper('ewcore')->getRequestRoute() . ' - ' . serialize($dataForKey)  . '-' . (int)Mage::app()->getRequest()->isSecure() . '-' . (int)Mage::helper('ewminify')->isDeferJsEnabled());
