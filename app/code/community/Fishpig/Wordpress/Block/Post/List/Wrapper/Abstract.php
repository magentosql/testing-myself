...
abstract class Fishpig_Wordpress_Block_Post_List_Wrapper_Abstract extends Mage_Core_Block_Template
{
	...
	public function __construct()
	{
		...
		foreach ($urlParts as $part) {
			$path = "wordpress/post/list_" . $part . ".phtml";
			if (!preg_match('#^(\w+/){1,2}\w+\.\w+$#',$path) || "" == $part) continue;
			$i++;
			if (!file_exists($basePath . $path)) continue;
			$templatePath = $path;
			break;
		}
		if ((1 == $i) && (file_exists($basePath . "wordpress/post/list_homepage.phtml"))) {
			$templatePath = "wordpress/post/list_homepage.phtml";
		}
		$this->setPostListTemplate($templatePath);
	}
