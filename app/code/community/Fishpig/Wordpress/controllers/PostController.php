...
class Fishpig_Wordpress_PostController extends Fishpig_Wordpress_Controller_Abstract
{
	...
	public function viewAction()
	{
		...
		try {
			$pc = $post->getParentCategories();
			foreach ($pc as $postCat) {
				array_push($handles, 'wordpress_post_view_cat' . $postCat->getId());
			}
		} catch (Exception $e) {}
		
		$this->_addCustomLayoutHandles($handles);
		...
		if (null != $post->getParentCategory()) {
			$category = $post->getParentCategory();
			
			$this->addCrumb('post_category', array('label' => $category->getName(), 'link' => $category->getUrl()));			
		}
