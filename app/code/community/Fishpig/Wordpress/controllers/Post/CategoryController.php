...
class Fishpig_Wordpress_Post_CategoryController extends Fishpig_Wordpress_Controller_Abstract
{
	...
	{		
		...
		$item = $category;
		while ($item &&  in_array("getParentTerm", get_class_methods($item)) && $item->getParentId()) {
			array_push($_layouts, 'wordpress_category_'.$item->getParentId());
			$item = $item->getParentTerm();
		}

		$this->_addCustomLayoutHandles(array_unique($_layouts));
