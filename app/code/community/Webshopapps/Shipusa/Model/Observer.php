...
class Webshopapps_Shipusa_Model_Observer extends Mage_Core_Model_Abstract
{
	...
	private function updateVolumes(&$shipBox) {
		...
		if ($shipBox['box_id']==0) {
			$shipBox['box_volume']=$shipBox['length']*$shipBox['width']*$shipBox['height'];
			if ($shipBox['box_volume']<5184) {
				$shipBox['box_volume']=10;
			}
		}
	...
	private function saveShipBoxes($product) {
		...
		if (!empty($data['insert'])) {
			foreach ($data['insert'] as $shipBox) {
				$shipBox['product_id'] = $product->getId();
				$write->insert($table, $shipBox);
			}
		}
	...
	public function saveOrderAfter($observer){
		...
		try {
			if (Mage::getStoreConfigFlag('shipping/shipusa/package_comment_history')) {
				$order = $observer->getEvent()->getOrder();
				$orderId = $order->getIncrementId();
				$packagesModel= Mage::getModel('shipusa/order_packages')->getCollection()
				->addOrderFilter($orderId);
				$boxes = $packagesModel->getData();

				$boxText = '';
				foreach ($boxes as $key=>$box) {
					$boxText .= 'Package #'.($key+1).'</br>';
					$boxText .= ': Length='.$box['length'];
					$boxText .= ': Width='.$box['width'];
					$boxText .= ': Height='.$box['height'];
					$boxText .= ': Weight='.$box['weight'].'</br>';
	...
	public function salesConvertQuoteToOrder($observer) {
		...
		try {
			...
			foreach ($packagesColl as $box) {
				$package = Mage::getModel('shipusa/order_packages')
				->setOrderId($orderId)
