    ...
    implements Mage_Shipping_Model_Carrier_Interface
{
    ...
    /**
     * @var Webshopapps_Upscalendar_Model_Usa_Shipping_Upstransit|null
     */
    ...
    protected function _addAllPackages(&$shipment,&$handProdFee) {
        ...
        $boxes=Mage::getSingleton('shipusa/dimcalculate')->getBoxes($this->_request->getAllItems(),
            $r->getMaxPackageWeight(),$r->getIgnoreFreeItems());
    ...
    protected function _parseDimResponse($debugData,$xmlResponse,$handProdFee)
    {
        $costArr = array();
        $priceArr = array();
        if (strlen(trim($xmlResponse))>0) {
            ...
            if($success===1){
                ...
                if ($this->getDebugFlag()) {
        			Mage::helper('wsacommon/log')->postMajor('usashipping','UPS Error Raised','');
        		}
             }
        }
        
        if ($this->getDebugFlag()) {
        	Mage::helper('wsacommon/log')->postMinor('usashipping','UPS Response Prices',$priceArr);
        }
        ...
        if (empty($priceArr)) {
            ...
            if (is_object($this->_upsTransitModel)) {
                return $this->_upsTransitModel->processRateResults();
            }
    ...
    public function getCode($type, $code='')
    {
        ...
        $codes = array(
            ...
            'container_description'=>array(
                ...
                'ULP'    => Mage::helper('usa')->__('UPS Package'),
            	'UT'     => Mage::helper('usa')->__('UPS Tube'),
            	'PAK'    => Mage::helper('usa')->__('UPS Pak'),
                ...
                'PAL'    => Mage::helper('usa')->__('UPS Pallet'),
            ),
