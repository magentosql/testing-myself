...
/**
    ...
    protected function _parseXmlTrackingResponse($trackingvalue, $response)
    {
    	if (!Mage::getStoreConfig('shipping/shipusa/active')) {
    		return Mage_Usa_Model_Shipping_Carrier_Fedex::_parseXmlTrackingResponse($trackingvalue,$response);
    	}
         $resultArr=array();
    ...
    protected function _doRatesRequest($purpose)
    {
        ...
        if ($response === null) {
            try {
            	...
            	if ($this->getDebugFlag()) {
			   		Mage::helper('wsacommon/log')->postCritical('usashipping','Fedex Soap Exception',$e->getMessage());
				}
            }
        } else {
    	...
    	if ($this->getDebugFlag()) {
        	Mage::helper('wsacommon/log')->postMinor('usashipping','Fedex Soap Request/Response',$debugData);
        	Mage::helper('wsacommon/log')->postMinor('usashipping','Handing Fee',$this->_handlingProductFee);
    	}
        return $response;
    }
